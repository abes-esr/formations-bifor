...
public class NbStagiaires extends Export {
	protected void lineToCSV(CSVWriter writer, ResultSet rs) throws SQLException {
		writer.writeNext(new String[]{rs.getString(1), Integer.toString(rs.getInt(2))});
	}
	
	protected String getQuery() {
		return "(SELECT s.IDTYPESESSION, COUNT(0) " +
				...
				"(SELECT s.IDTYPESESSION, COUNT(0) " +
				"FROM INSCRIPTION i, SESSIONS s " +
				"WHERE " +
					"s.DATEDEBUT BETWEEN ADD_MONTHS( ? , -1) AND ? " + 
