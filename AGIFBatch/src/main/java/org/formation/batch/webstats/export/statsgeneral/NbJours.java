...
public class NbJours extends Export {
	protected void lineToCSV(CSVWriter writer, ResultSet rs) throws SQLException {
		Integer days = (int) Math.round(rs.getFloat(2) * 2.0);
		writer.writeNext(new String[]{rs.getString(1), days.toString()});
	}
	
	protected String getQuery() {
		return "(SELECT s.IDTYPESESSION, SUM(NBJOURS) " +
				...
				"(SELECT s.IDTYPESESSION, SUM(NBJOURS) " +
				"FROM INSCRIPTION i, SESSIONS s " +
				"WHERE " +
					"s.DATEDEBUT BETWEEN ADD_MONTHS( ? , -1) AND ? " + 
